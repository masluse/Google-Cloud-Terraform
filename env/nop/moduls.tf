...
module "virtual_machine" {
  ...
  backup_policy         = module.disk_policy.google_compute_resource_policy.name # Backup policy associated with the VM.
  service_account_email = module.service_account.google_service_account.email        # Service account email associated with the VM.
...
module "disk" {
  ...
  backup_policy = module.disk_policy # Backup policy to be associated with the disk.
  instance_id   = module.virtual_machine        # ID of the VM instance to which the disk will be attached.
...
module "ansible2" {
  ...
  vm_name        = local.vm_name[1]             # Public IP of the provisioned VM.
  vm_zone        = module.vm1.google_compute_instance.zone
...
output "gcloud-connect" {
  ...
  value       = "gcloud compute ssh ${local.vm_name}"
}
